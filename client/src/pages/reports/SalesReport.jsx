...
const SalesReport = () => {
  ...
  useEffect(() => {
    fetchReport();
  }, [search, fromDate, toDate, sortField, sortOrder]);
    ...
    <div className="sales-report-page">
      ...
      <div className="glass-card filters-panel" style={{ padding: '18px 24px', marginBottom: '20px' }}>
        ...
        <div className="flex" style={{ justifyContent: 'flex-end', marginTop: '14px', gap: '8px' }}>
          <button className="btn btn-secondary btn-sm" onClick={() => { setSearch(''); setFromDate(''); setToDate(''); }}>
            Reset Filters
          </button>
        </div>
      </div>
        ...
        <div className="table-container">
          <table className="custom-table">
            ...
            <tbody>
              ...
              {orders.length === 0 && (
                <tr>
                  <td colSpan="6" style={{ textAlign: 'center', padding: '24px 0', color: 'var(--text-muted)' }}>No reports match the selected filters</td>
                </tr>
              )}
            </tbody>
          </table>
